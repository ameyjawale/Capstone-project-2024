...
-- Delete rows with null or blank values in transactions table
...
-- Delete rows with null or blank values in products table
DELETE FROM products
WHERE product_code IS NULL OR product_code = ''
OR product_type IS NULL OR product_type = '';

-- Delete rows with null or blank values in markets table
DELETE FROM markets
WHERE markets_code IS NULL OR markets_code = ''
OR markets_name IS NULL OR markets_name = ''
OR zone IS NULL OR zone = '';

-- Delete rows with null or blank values in date table
DELETE FROM date
WHERE date IS NULL
OR cy_date IS NULL
OR year IS NULL
...
-- Delete rows with null or blank values in customers table
DELETE FROM customers
